...
void test_msg_init ()
{
    ...
    TEST_ASSERT_EQUAL_INT (0, zmqp_msg_size (&msg));
...
void test_msg_init_size ()
{
    ...
    TEST_ASSERT_EQUAL_INT (6, zmqp_msg_size (&msg));
    memcpy (zmqp_msg_data (&msg), data, 6);
    TEST_ASSERT_EQUAL_STRING_LEN (data, zmqp_msg_data (&msg), 6);
    TEST_ASSERT_SUCCESS_ERRNO (zmq_msg_close (&msg));

    zmq_msg_t msg2;
    TEST_ASSERT_SUCCESS_ERRNO (zmq_msg_init_size (&msg2, 0));
    TEST_ASSERT_EQUAL_INT (0, zmqp_msg_size (&msg2));
...
void test_msg_init_buffer ()
{
    ...
    TEST_ASSERT_EQUAL_INT (6, zmqp_msg_size (&msg));
    TEST_ASSERT (data != zmqp_msg_data (&msg));
    TEST_ASSERT_EQUAL_STRING_LEN (data, zmqp_msg_data (&msg), 6);
    TEST_ASSERT_SUCCESS_ERRNO (zmq_msg_close (&msg));
