...
// TODO: find a better home for these. Must be included here.
...
constexpr std::unique_ptr<T> make_unique_nothrow (Args &&...args)
...
template <class T,
          class... Args,
          std::enable_if_t<!std::is_array_v<T>, int> = 0>
constexpr std::shared_ptr<T> make_shared_nothrow (Args &&...args)
  noexcept (noexcept (T (std::forward<Args> (args)...)))
{
    return std::shared_ptr<T> (new (std::nothrow)
                                 T (std::forward<Args> (args)...));
}
}
