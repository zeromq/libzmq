...
{
    zmq_assert (_addr->protocol == protocol_name::hvsocket);
}

zmq::hvsocket_connecter_t::~hvsocket_connecter_t ()
{
    zmq_assert (!_connect_timer_started);
}

void zmq::hvsocket_connecter_t::process_term (int linger_)
{
...
void zmq::hvsocket_connecter_t::in_event ()
{
...
void zmq::hvsocket_connecter_t::out_event ()
{
    if (_connect_timer_started) {
        cancel_timer (connect_timer_id);
        _connect_timer_started = false;
    ...
    create_engine (
      fd, zmq::hvsocket_connecter_t::get_socket_name (fd, socket_end_local));
}
...
void zmq::hvsocket_connecter_t::timer_event (int id_)
{
...
void zmq::hvsocket_connecter_t::start_connecting ()
{
...
void zmq::hvsocket_connecter_t::add_connect_timer ()
{
...
int zmq::hvsocket_connecter_t::open ()
{
    zmq_assert (_s == retired_fd);

    //  Resolve the address
    if (_addr->resolved.hvsocket_addr != NULL) {
        LIBZMQ_DELETE (_addr->resolved.hvsocket_addr);
    }
    ...
    zmq_assert (_addr->resolved.hvsocket_addr != NULL);
...
zmq::fd_t zmq::hvsocket_connecter_t::connect ()
{
